...
x := []int{1, 2, 3, 4, 5}
var y []int
y = append(x, 6)     // если len(x) и cap(x) равны, при добавлении элемента производится аллокация нового массива?
y = append(x, 7)     // тот же вопрос
x = y
y = append(x, 8)     // когда cap достаточно при добавлении элемента, производится аллокация нового массива?
fmt.Println(x, y)
```
...
fmt.Println(x, y)
for i := range 5 {
	y = append(y, i+10)      // при исчерпании cap проихойдет аллокация нового массива?
}
fmt.Println(x, y)
```
