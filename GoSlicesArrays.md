...
\
Какие операции над неинициализированным слайсом можно совершить? Можно ли итерироватся по пустому слайсу с помощью `range`?\
\
...
\
Какая из функций "нуллификации" массива отрабатывает быстрее с минимальным потреблением памяти?
```go
func() {
	arr := [5]int{1, 2, 3, 4, 5}
	for i := range arr {
...
func(s []int) {          // переменные передаются по ссылке или по значению?
	s[0] *= 2
	s = append(s, 7)     // происходит ли затемнение переданной переменной?
	s[1] /= 2
}(s1)
